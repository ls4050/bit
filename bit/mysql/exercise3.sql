...
-- 	---------------------------------------
-- 	 6명                    		 13명
select concat(count(comm), '명')
	,concat(count(deptno), '명')
from emp;
...
-- concat함수는 (1, 2) 1번인자가 null이면 그냥 뒤에 다무시하고 null반환한다
select ifnull(concat(deptno, '번 부서'), '미정') 부서정보
       , concat(count(*),'명') 직원명수
from emp
group by deptno;
...
-- 숫자 기반 SORT
select concat(year(hiredate), '년') 입사년도
...
-- 문자열 기반 SORT
-- 한자리씩  한자리씩 검사함
-- 10 1 1 2 면 2가 가장큰거로 나옴 첫째자리 2가 가장 크므로
select concat(year(hiredate), '년') 입사년도
	   , concat(count(*), '명') 입사직원수
from emp
group by year(hiredate)
...
-- 	  CLERK		950.00
select job 직무
       , round(avg(ifnull(sal,0)+ifnull(comm,0)), 2) 평균연봉
from emp
where deptno = 30
group by job; 
